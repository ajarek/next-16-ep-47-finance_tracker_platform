...
/**
 * Modal szybkiej autoryzacji i rejestracji 7-dniowego darmowego okresu próbnego.
 */
          ...
          >
            ...
            <div className='mb-6'>
              ...
              <p className='text-xs sm:text-sm text-on-surface-variant mt-1.5'>
                Uzyskaj natychmiastowy dostęp do wszystkich funkcji analizy
                budżetu i synchronizacji Firebase.
              </p>
            </div>
            ...
            <div className='space-y-3'>
              <button
                type='button'
                onClick={() => {
                  alert("Logowanie przez Google Auth")
                  onClose()
                }}
              ...
              <button
                ...
                onClick={() => {
                  onClose()
                  const target = document.getElementById("cennik")
                  if (target) target.scrollIntoView({ behavior: "smooth" })
                }}
                className='w-full py-3 px-4 rounded-xl bg-surface-container-lowest hover:bg-surface-container border border-border text-on-surface font-semibold text-xs sm:text-sm flex items-center justify-center gap-3 transition-all hover:scale-[1.01] cursor-pointer'
              >
                <Mail className='w-4 h-4 text-primary' />
                <span>Zarejestruj adresem e-mail</span>
              </button>
            </div>
